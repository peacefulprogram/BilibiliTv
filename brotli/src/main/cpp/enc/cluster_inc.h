...
BROTLI_INTERNAL void FN(BrotliCompareAndPushToQueue)(
        ...
        size_t *num_pairs)CODE({
                                   ...
                                   p.idx1 = idx1;
                                   p.idx2 = idx2;
                                   p.cost_diff = 0.5 * ClusterCostDiff(cluster_size[idx1],
                                                                       cluster_size[idx2]);
...
BROTLI_INTERNAL void FN(BrotliHistogramRemap)(const HistogramType *in,
                                              size_t in_size, const uint32_t *clusters,
                                              size_t num_clusters,
...
BROTLI_INTERNAL size_t FN(BrotliHistogramReindex)(MemoryManager *m,
                                                  ...
                                                  size_t length)CODE({
                                                                         ...
                                                                         uint32_t *new_index = BROTLI_ALLOC(
                                                                                 m, uint32_t,
                                                                                 length);
                                                                         uint32_t next_index;
                                                                         HistogramType *tmp;
...
BROTLI_INTERNAL void FN(BrotliClusterHistograms)(
        ...
        uint32_t *histogram_symbols)CODE({
                                             uint32_t *cluster_size = BROTLI_ALLOC(m, uint32_t,
                                                                                   in_size);
                                             uint32_t *clusters = BROTLI_ALLOC(m, uint32_t,
                                                                               in_size);
                                             size_t num_clusters = 0;
                                             const size_t max_input_histograms = 64;
                                             size_t pairs_capacity =
                                                     max_input_histograms * max_input_histograms /
                                                     2;
                                             /* For the first pass of clustering, we allow all pairs. */
                                             HistogramPair *pairs = BROTLI_ALLOC(m, HistogramPair,
                                                                                 pairs_capacity +
                                                                                 1);
                                             size_t i;
                                             ...
                                             for (i = 0; i < in_size; ++i) {
                                                 out[i] = in[i];
                                                 out[i].bit_cost_ = FN(BrotliPopulationCost)(
                                                         &in[i]);
                                             ...
                                             for (i = 0; i < in_size; i += max_input_histograms) {
                                                 size_t num_to_combine =
                                                         BROTLI_MIN(size_t, in_size - i,
                                                                    max_input_histograms);
                                                 size_t num_new_clusters;
                                                 size_t j;
                                                 for (j = 0; j < num_to_combine; ++j) {
                                                     clusters[num_clusters + j] = (uint32_t) (i +
                                                                                              j);
                                                 }
                                                 num_new_clusters =
                                                         FN(BrotliHistogramCombine)(out,
                                                                                    cluster_size,
                                                                                    &histogram_symbols[i],
...

                                             {
                                                 ...
                                                 BROTLI_ENSURE_CAPACITY(
                                                         m, HistogramPair, pairs, pairs_capacity,
                                                         max_num_pairs + 1);
                                                 if (BROTLI_IS_OOM(m)) return;

                                                 /* Collapse similar histograms. */
                                                 num_clusters = FN(BrotliHistogramCombine)(out,
                                                                                           cluster_size,
                                                                                           histogram_symbols,
                                                                                           clusters,
                                                                                           pairs,
                                             ...
                                             FN(BrotliHistogramRemap)(in, in_size, clusters,
                                                                      num_clusters,
                                                                      out, histogram_symbols);
                                             BROTLI_FREE(m, clusters);
                                             /* Convert the context map to a canonical form. */
                                             *out_size = FN(BrotliHistogramReindex)(m, out,
                                                                                    histogram_symbols,
                                                                                    in_size);
                                             if (BROTLI_IS_OOM(m)) return;
                                         })
