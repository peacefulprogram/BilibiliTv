...
static BROTLI_INLINE void UpdateZopfliNode(ZopfliNode *nodes, size_t pos,
                                           size_t start_pos, size_t len, size_t len_code,
                                           size_t dist,
...
static size_t ZopfliIterate(size_t num_bytes, size_t position,
                            ...
                            const BrotliEncoderParams *params, const size_t gap,
...
size_t BrotliZopfliComputeShortestPath(MemoryManager *m, size_t num_bytes,
                                       size_t position, const uint8_t *ringbuffer,
                                       size_t ringbuffer_mask,
                                       const BrotliEncoderParams *params,
                                       const int *dist_cache, HasherHandle hasher,
                                       ZopfliNode *nodes) {
    const size_t max_backward_limit = BROTLI_MAX_BACKWARD_LIMIT(params->lgwin);
    const size_t max_zopfli_len = MaxZopfliLen(params);
...
void BrotliCreateHqZopfliBackwardReferences(MemoryManager *m, size_t num_bytes,
    ...
    for (i = 0; i < 2; i++) {
        ...
        *num_commands += ZopfliIterate(num_bytes, position, ringbuffer,
                                       ringbuffer_mask, params, gap, dist_cache, &model,
                                       num_matches, matches,
                                       nodes);
        BrotliZopfliCreateCommands(num_bytes, position, nodes, dist_cache,
                                   last_insert_len, params, commands, num_literals);
